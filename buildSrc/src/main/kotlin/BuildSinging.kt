...
sealed class BuildSinging(val name:String) {
    abstract fun create(namedDomainObjectContainer : NamedDomainObjectContainer<out ApkSigningConfig>)

    //Release
    class Release(private val project :Project) :BuildSinging(SigningTypes.RELEASE){
    ...
    //ReleaseExternalQA
    class ReleaseExternalQA (private val project:Project):BuildSinging(SigningTypes.RELEASE_EXTERNAL_QA){
        override fun create(namedDomainObjectContainer: NamedDomainObjectContainer<out ApkSigningConfig>) {
            namedDomainObjectContainer.create(name){
                storeFile= File(project.getLocalProperty("qa_key.store"))
                storePassword=project.getLocalProperty("qa_store.password")
                keyAlias=project.getLocalProperty("qa_key.alies")
    ...
    //Debug
    class Debug (private val project: Project):BuildSinging( SigningTypes.DEBUG){
        override fun create(namedDomainObjectContainer: NamedDomainObjectContainer<out ApkSigningConfig>) {
